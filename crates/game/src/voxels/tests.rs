...
use super::{Blocks, voxel_chunk::VoxelChunk};
use crate::{utils::BlockIter, voxels::CHUNK_VOL};
use chunk_serde::CompressedChunkData;

#[test]
fn chunk_compression() {
    let empty = VoxelChunk::empty();
    let solid = VoxelChunk::solid(Blocks::Copper);
    ...
    assert_eq!(VoxelChunk::decompress(&empty_cop), empty);
    // check solid.comp == solid.decomp
    assert_eq!(VoxelChunk::decompress(&solid_cop), solid);
...
fn chunk_serde() {
    ...
    macro_rules! test {
        ($generator:expr) => {
            ...
            comp.into_vec(&mut data);
            (out, len) = CompressedChunkData::from_slice(&data);
    ...
    test!(VoxelChunk::decompress(&raw));
    let raw_len = len;
